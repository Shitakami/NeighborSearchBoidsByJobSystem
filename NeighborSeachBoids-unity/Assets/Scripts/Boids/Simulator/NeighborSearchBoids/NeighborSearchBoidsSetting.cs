...
namespace Boids.Settings
{
    ...
    public class NeighborSearchBoidsSetting : ScriptableObject
    {
        ...
        [Space(20)]
        [SerializeField] private float _neighborSearchGridScale;
        ...
        public Vector3 SimulationAreaScaleHalf => _simulationAreaScale / 2; // MEMO: 計算では 1/2 の方が書きやすいため
        public float AvoidSimulationAreaWeight => _avoidSimulationAreaWeight;

        public float MaxSpeed => _maxSpeed;
        public float MaxSteerForce => _maxSteerForce;

        public float NeighborSearchGridScale => _neighborSearchGridScale;

        public float3 MinNeighborSearchGridPoint => _simulationAreaCenter - SimulationAreaScaleHalf;

        public int3 NeighborSearchGridCount => (int3) math.ceil( new float3(_simulationAreaScale) / _neighborSearchGridScale);
